...
Note: no separate composite index on (cik, ticker) is needed — SQLite creates
a B-tree index automatically to enforce the UNIQUE constraint.

Revision ID: e421f7a39fcf
...
def upgrade() -> None:
    ...
    with op.batch_alter_table('underlying_securities', schema=None) as batch_op:
        ...
        # sqlite_autoindex that create_all() generated.  SQLite enforces this
        # with an implicit B-tree index, so no separate composite index is added.
