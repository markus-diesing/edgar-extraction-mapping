...
# Documentation landing page: http://localhost:8000/docs/index.html
```
...
4. `docs/dev/CLAUDE_CODE_BOOTSTRAP.md` — session bootstrap checklist
...
| Method | Path | Description |
|--------|------|-------------|
| POST | `/api/docs/chat` | Chat with embedded documentation assistant (used by user_manual.html) |
| GET  | `/api/docs/manifest` | Returns metadata for all documentation files (used by docs/index.html landing page) |
| GET  | `/docs/{path}` | Static file mount serving all files under docs/ directory |
...
### Financial Glossary
`docs/research/financial_glossary.md` is loaded into the extraction system prompt (mtime-cached). Add domain terminology here to improve extraction accuracy without code changes.

### Three-State Classification

The classifier produces one of three status values:

| Status | Confidence | Meaning |
|--------|-----------|---------|
| `classified` | ≥ 0.80 | High-confidence match — extraction can proceed immediately |
| `needs_classification_review` | 0.60–0.79 | Borderline match — human confirmation required before extraction |
| `needs_review` | unknown / < 0.60 | No confident model match — reviewer must set model manually |

`POST /api/classify/{id}/confirm` promotes `needs_classification_review` → `classified`, sets confidence to 1.0, and writes a `ClassificationFeedback` record. Returns 422 if the filing is not in the `needs_classification_review` state.

The CUSIP hint from `CUSIP_PRISM_Mapping.xlsx` is **supplementary only** — it informs but does not anchor the classifier. Classification is primarily based on document content.
